...
export function AnnonceClient({ id, initialProperty }: { id: string; initialProperty?: Property }) {
  ...
  const { data: property, isLoading } = useProperty(id, initialProperty);
  ...
  // Le numéro n'est jamais renvoyé à un visiteur non connecté (API publique, voir
  // PropertiesService.toPublicView) : le bouton "Contacter" (qui mène à la connexion) doit donc
  // s'afficher aussi sans numéro tant qu'on n'est pas connecté.
  const hasContact = Boolean(property.contactPhone) || !isAuthenticated;
